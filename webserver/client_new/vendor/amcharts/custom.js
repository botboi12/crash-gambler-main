function CustomChart(startingFrom, cum, chartData) {
  ...
  chartData.forEach(function (entry, i) {
    var profit =
      (entry.cash_out ? entry.cash_out : 0) +
  ...
  AmCharts.ready(function () {
    function frmt(x) {
      ...
      var r =
        ...
        entry.bet.toFixed() +
        " satoshis</td></tr>" +
        "<tr><th>Crash At:</th><td>" +
        (typeof entry.game_crash !== "undefined"
          ? (entry.game_crash / 100).toFixed(2) + "x"
          : "?") +
        "</td></tr>" +
        "<tr><th>Cashed Out:</th><td>" +
        (entry.cash_out ? (entry.cash_out / entry.bet).toFixed(2) + "x" : "-") +
        "</td></tr>" +
        "<tr><th>Bonus: </th><td>" +
        (entry.bonus ? entry.bonus.toFixed(2) : 0) +
        " satoshis</td></tr>" +
        "<tr><th>Profit:</th><td><b>" +
        profit.toFixed(2) +
        " satoshis</b></td></tr>" +
        "</table>";
      return r;
