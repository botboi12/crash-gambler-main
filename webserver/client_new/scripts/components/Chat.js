define([
  ...
  return React.createClass({
    ...
    componentDidUpdate: function (prevProps, prevState) {
      if (this.state.evName === "joined") {
        //On join scroll to the bottom
        var msgsNode = this.refs.messages.getDOMNode();
        msgsNode.scrollTop = msgsNode.scrollHeight;
      } else if (ChatEngine.history.length != this.listLength) {
        ...
        if (this.refs.messages) {
          ...
          var scrollBottom =
            msgsBox.scrollHeight - msgsBox.offsetHeight - msgsBox.scrollTop;
      ...
      switch (cmd) {
        case "ignore":
          if (ChatEngine.username === rest) {
            ...
            !this.state.ignoredClientList.hasOwnProperty(rest.toLowerCase())
          ) {
            ChatActions.ignoreUser(rest);
            ChatActions.showClientMessage("User " + rest + " ignored");
          } else
            ChatActions.showClientMessage(
              "User " + rest + " was already ignored"
            );
        ...
        case "unignore":
          if (Clib.isInvalidUsername(rest)) {
            ...
            ChatActions.showClientMessage(
              "User " + rest + " was already approved"
            );
    ...
    render: function () {
      ...
      if (ChatEngine.username && ChatEngine.state == "JOINED")
        chatInput = D.input(
          //Input is not binded due to slowness on some browsers
          {
    ...
    _renderMessage: function (message, index) {
      ...
      switch (message.type) {
        case "say":
          ...
          if (
            this.state.ignoredClientList.hasOwnProperty(
          ...
          if (message.bot || /^!/.test(message.message)) {
            ...
            if (this.state.botsDisplayMode === "none") return;

            pri += " msg-bot";

            if (this.state.botsDisplayMode === "greyed") pri += " bot-greyed";
          }
          ...
          return D.li(
            ...
            D.span({
              ...
              dangerouslySetInnerHTML: {
                __html: Autolinker.link(escapeHTML(message.message), {
                  truncate: 50,
        ...
        case "mute":
          ...
          return D.li(
            { className: pri, key: "msg" + index },
            D.a(
              { href: "/user/" + message.username, target: "_blank" },
              "<" + message.username + ">"
            ),
        ...
        case "unmute":
          ...
          return D.li(
            { className: pri, key: "msg" + index },
            D.a(
              { href: "/user/" + message.moderator, target: "_blank" },
              "*** <" + message.moderator + ">"
            ),
