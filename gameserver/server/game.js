...
function Game(lastGameId, lastHash, bankroll, gameHistory) {
  ...
  function runTick() {
    ...
    if (self.forcePoint <= at && self.forcePoint <= self.crashPoint) {
      self.cashOutAll(self.forcePoint, function (err) {
        console.log(
          "Just forced cashed out everyone at: ",
          self.forcePoint,
          " got err: ",
          err
        );
    ...
    if (at > self.crashPoint) endGame(false);
    // oh noes, we crashed!
    else tick(elapsed);
  }
  ...
  function endGame(forced) {
    ...
    function dbTimeout() {
      dbTimer = setTimeout(function () {
        console.log(
          "Game",
          gameId,
          "is still ending... Time since crash:",
    ...
    db.endGame(gameId, bonuses, function (err) {
      ...
      if (self.gameShuttingDown) self.emit("shutdown");
      else setTimeout(runGame, crashTime + afterCrashTime - Date.now());
    });
...
Game.prototype.placeBet = function (user, betAmount, autoCashOut, callback) {
  ...
  if (
    lib.hasOwnProperty(self.pending, user.username) ||
    lib.hasOwnProperty(self.players, user.username)
  )
...
Game.prototype.doCashOut = function (play, at, callback) {
  ...
  db.cashOut(play.user.id, play.playId, won, function (err) {
    if (err) {
      console.log(
        "[INTERNAL_ERROR] could not cash out: ",
        username,
        " at ",
        at,
        " in ",
...
Game.prototype.runCashOuts = function (at) {
  ...
  Object.keys(self.players).forEach(function (playerUserName) {
    ...
    if (
      play.autoCashOut <= at &&
      play.autoCashOut <= self.crashPoint &&
      play.autoCashOut <= self.forcePoint
    ) {
...
Game.prototype.cashOut = function (user, callback) {
  ...
  if (at > self.crashPoint) return callback("GAME_ALREADY_CRASHED");
...
Game.prototype.cashOutAll = function (at, callback) {
  ...
  Object.keys(self.players).forEach(function (playerName) {
    ...
    if (play.status === "PLAYING") {
      tasks.push(function (callback) {
        if (play.status === "PLAYING") self.doCashOut(play, at, callback);
        else callback();
      });
    }
  });
  ...
  async.parallelLimit(tasks, 4, function (err) {
    if (err) {
      console.error(
        "[INTERNAL_ERROR] unable to cash out all players in ",
        self.gameId,
        " at ",
        at
      );
  ...
  slideSameStoppedAt(
    ...
    function (listOfRecords, cashOutAmount, totalBetAmount) {
      ...
      for (var i = 0; i < listOfRecords.length; ++i) {
        var toAlloc = Math.round(
          (listOfRecords[i].bet / totalBetAmount) * toAllocAll
        );
